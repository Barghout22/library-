...
function displayMyBooks(myLibrary) {
  for (let i = 0; i < myLibrary.length; i++) {
    ...
    pageNumber.textContent = myLibrary[i].pageNumber +" pages";
...
const errorSpan = document.querySelector(".errorMesage");
submittNewBook.addEventListener("click", () => {
  if (
    !placeholderObject.book ||
    !placeholderObject.author ||
    !placeholderObject.pages ||
    placeholderObject.book === " " ||
    placeholderObject.author === " "
  ) {
    errorSpan.textContent = "please fill out all fields to continue";
  } else {
    addBookToLibrary(
      new Book(
        placeholderObject.book,
...
Inputs.forEach((input) =>
  input.addEventListener("blur", () => sortInputValue(input))
);
